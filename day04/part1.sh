...
for (( i=0; i < $max_guard; i++ )); do
  if [ -n "${guards[$i,d]}" ]; then
    sleeptable="${sleeptable}${guards[$i,d]} $i"$'\n' # uses $'\n' to add a newline
  fi
done

echo "$sleeptable" | sort -r
longest_sleeper=$(echo "$sleeptable" | sort -r | head -n 1 | awk '{print $2}')
echo
echo "Guard $longest_sleeper sleeps the longest"
echo

for (( i=0; i < 60; i++ )); do
  if [ -n "${guards[$longest_sleeper,$i]}" ]; then
    timetable=${timetable}$(printf '%02d %02d' ${guards[$longest_sleeper,$i]} $i)$'\n'
  fi
done

echo "$timetable" | sort -r
best_time=$(echo "$timetable" | sort -r | head -n 1 | awk '{print $2}')
echo
echo "Best time: $best_time"
echo
echo "Result: $(( $longest_sleeper * $best_time ))"
